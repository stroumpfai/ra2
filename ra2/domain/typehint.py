...
The suffix rule runs **first**; value-shape inspection only decides what the
suffix leaves open. An `Ausw` column (RADIS) or a `* UAP` column (Astrana) is
`enum` even when every value looks like an integer — there is a test for each
that says so.
"""
...
def infer_type_hint(column_name: str, values: Iterable[str]) -> TypeHint:
    ...
    1. `*Ausw` (RADIS) or `* UAP` (Astrana) -> `ENUM`, unconditionally.
