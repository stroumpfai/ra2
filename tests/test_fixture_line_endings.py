...
The repository answers it once, globally: `.gitattributes` sets `* -text`, so
no path is converted in either direction and every file checks out with the
bytes it was committed with — LF where the generator wrote LF, CRLF where it
wrote CRLF. That is stronger than pinning each family, and it is why the
per-family expectations below all read the same. A narrow scheme naming
`eol=lf` for the LF families was the alternative and is worse than either half
of it alone: those lines would match *after* `* -text`, and last-match-wins
would re-enable conversion on precisely the paths whose bytes are asserted.
"""
...
def test_no_byte_asserted_fixture_is_left_to_core_autocrlf():
    ...
    assert unpinned == [], (
        ...
        "`.gitattributes` entry (`* -text` covers every path; a narrower rule "
        f"would have to match after it): {unpinned}"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("tests/fixtures/codelists/hazards/c01_minimal_valid/codelist.json", {"text": "unset"}),
        ("tests/fixtures/prompts/hazards/p01_valid_all_slots/template.txt", {"text": "unset"}),
        ("tests/fixtures/golden/import_report_hazards.json", {"text": "unset"}),
    ],
)
def test_each_family_resolves_the_attribute_its_generator_needs(relative, expected):
    ...
    All four read `text: unset` because `* -text` is the repository's one
    rule, not because the families agree by accident: each is checked out
    with the bytes it was committed with, which is what byte-identity needs
