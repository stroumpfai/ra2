...
run(... , prompt_template_id, prompt_template_fingerprint, status, error,
     llm_parallel_calls, model_size_bytes)
          ...
          -- else 1; NOT NULL
          -- model_size_bytes (SD41): the catalogue's size for this tag at
          -- launch, the number the Models card showed beside the tick;
          -- NULLABLE, no backfill — a run from before the revision has none,
          -- and the ranking renders that as an em dash, never as 0
...
- **An entry in the map is a measurement, not a preference, and the launch
  ...
  queue wait runs against `RA2_LLM_TIMEOUT_S`. The launch logs each model's
  decision (`parallel=1 (map=4, gate=digest)`: ids, tags and a code only), and
  the run-start log line prints the pinned `parallel=N` beside `timeout=`.
...
one the tool applies". **VRAM is the run's pinned `model_size_bytes`**
(**SD41**), rendered both in its own column and in the Model column's
`digest · size` sub-line, which is what makes the design's "must agree with
the model sub-line" a property rather than a promise; a run launched before
that column existed renders `—` in both. **Time per record and parallel calls
join them** (**SD38**). A model whose run had `llm_parallel_calls > 1` answers each call
