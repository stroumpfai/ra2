...
4. **Models** — a `.card` holding a scroll well **capped at 4 visible rows (`max-height:196px`,
   ...
   **Since SD40, a third line** under the size line, mono 10.5px, says what this host has measured about
   ...
   `qualified` in `--ink3` "seed F1 0.895 · 1.7 s/rec · entities 0 % · ~1 h 25 m for 3 000", plus
   ...
   duration is left out when neither an evaluation scope nor a corpus is known. **No "recommended"
   badge** — the seed can't separate the leaders (`docs/choosing-models.md` §3). The row grows from 49px
   to 63px, so the well that shows **4 visible rows** grows from 196px to **252px**; the rule is the row
...
version list well 530px = 10 × 53px rows; models well 252px = 4 rows (196px before SD40 added the qualification line); resolved-prompt body 210px;
